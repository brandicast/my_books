...
4. **開始使用**
   前端預設會執行在 port `6666`。請打開您的瀏覽器並訪問 `http://localhost:6666`。

## 功能亮點
- **離線支援與自動更新**：作為一款 PWA，即使在網路不佳狀況下依然可開啟；並且當有新版部署時，Service Worker 會自動清除舊緩存並熱重載。
- **最佳化安裝體驗**：針對 Android 與 iOS 裝置分別實作了智慧安裝提示（Install Banner），一進站即可按照指引將 APP 加入桌面，享受全螢幕的 Native 體驗。
